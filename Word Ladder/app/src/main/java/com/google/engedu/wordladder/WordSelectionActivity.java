...
public class WordSelectionActivity extends AppCompatActivity {
    ...
    public boolean onStart(View view) {
        ...
        ArrayList<ArrayList<String>> paths = dictionary.findPath(
        ...
        if (paths != null) {
            ...
            intent.putExtra("noOfPaths",paths.size());
            intent.putExtra("pathLength",paths.get(0).size());
            intent.putExtra("Start",startWordView.getText().toString().toLowerCase());
            intent.putExtra("End",endWordView.getText().toString().toLowerCase());

            for(ArrayList<String> path:paths)
            intent.putExtra("dict"+paths.indexOf(path),path);
